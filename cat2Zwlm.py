...
# Returns the position of a multipole (l,m) in the array Ylm defined below.
# Note that the array only contains entries with m>0, so for (l,m) with m<0 this returns 
# (l,-m), and the conjugate * and factors (-1)^m  must be applied to the value 
# returned by Ylm:
def lm2i(l, m):
    if m>=0:
        return l*(l+1)/2+m
...
# This includes only m>0, arranjed like (l,m) = (0,0), (1,0), (1,1), (2,0), (2,1), (2,2)... 
...
for q in range (-qmax,0):
    ...
    for l in range(0,lmax+1):
        for m in range (-l,0):
            Nqlm1   = (-1)**-m * np.sum(fZq*Ylm[lm2i(l,m)])
            Nqlm.append( [q,l,m, Nqlm1.real, Nqlm1.imag] )
        for m in range(0,l+1):
            Nqlm1   = np.sum(fZq*np.conj(Ylm[lm2i(l,m)]))
            Nqlm.append( [q,l,m, Nqlm1.real, Nqlm1.imag] )
# Compute q>=0 modes:
